...
const temporaryAuthors = [
  { name: 'Katrina Uychaco', login: 'kuychaco', email: 'kuychaco@users.noreply.github.com' },
  { name: 'Neha Batra', login: 'nerdneha', email: 'neha@github.com' },
  { name: 'Markus Olsson', login: 'niik', email: 'j.markus.olsson@github.com' },
  { name: 'Josh Abernathy', login: 'joshaber', email: 'joshaber@github.com' },
]
...
export default class CommitView extends React.Component {
  ...
  render() {
    ...
    return (
      <div className="github-CommitView">
        ...
        <div onKeyDown={e => console.log(e)} className="native-key-bindings">
        <Select
          className="github-CommitView-coAuthorEditor input-textarea native-key-bindings"
          placeholder="Co-Authors"
          arrowRenderer={null}
          options={temporaryAuthors}
          labelKey="name"
          valueKey="email"
          ...
          value={this.state.selectedCoAuthors}
          multi={true}
        />
        </div>
  ...
  matchAuthors(authors, filterText, currentValues) {
    // console.log(authors, filterText, currentValues)
    return authors
      .filter(x => !currentValues || currentValues.indexOf(x) === -1)
      .filter(x => `${x.name}${x.login}`.indexOf(filterText) !== -1)
  }

  renderCoAuthorListItem(author) {
    return (
      <div className="github-CommitView-coAuthorEditor-selectListItem">
        <span className="github-CommitView-coAuthorEditor-username">{author.login}</span>
        <span className="github-CommitView-coAuthorEditor-name">{author.name}</span>
      </div>
    )
  }
